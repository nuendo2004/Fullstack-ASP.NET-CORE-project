...
import InverseLogo from "assets/images/brand/logo/immersed-spiral-logo.png";
...
const NavbarVertical = (props) => {
  ...
  const filterGrandChildrenRoles = (grandChildrenArray) => {
    if (!grandChildrenArray.roles) {
      return true;
    } else {
      return grandChildrenArray.roles.some((role) =>
        user.currentUser.roles.includes(role)
      );
    }
  };

  const filterChildrenRoles = (filterChildren) => {
    if (!filterChildren.children) {
      if (!filterChildren.roles) {
        return true;
      } else {
        return filterChildren.roles.some((role) =>
          user.currentUser.roles.includes(role)
        );
      }
    } else {
      let userFilteredGrandchildren = filterChildren.children.filter(
        filterGrandChildrenRoles
      );
      filterChildren.children = userFilteredGrandchildren;
  ...
  const filterMenuItems = (filteredDashboard) => {
    if (!filteredDashboard.children) {
      if (!filteredDashboard.roles) {
        return true;
      } else {
        return filteredDashboard.roles.some((role) =>
          user.currentUser.roles.includes(role)
        );
      }
    } else {
      const dashboardItems =
        filteredDashboard.children.filter(filterChildrenRoles);
      filteredDashboard.children = dashboardItems;
      if (filteredDashboard.children.length > 0) {
        return true;
      } else {
        return false;
      }
  ...
  let userDashboardMenu = DashboardMenu.filter(filterMenuItems);
      ...
      <SimpleBar style={{ maxHeight: "100vh" }}>
        <div className="nav-scroller">
          <Link className="navbar-brand" to="/">
            <Image src={InverseLogo} alt="" className="mb-4 w-25 h-25" />
            <h3 className="d-inline pull-right text-white p-2">
        ...
        >
          {userDashboardMenu.map(function (menu, index) {
            if (menu.grouptitle) {
              return (
